...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return {
    ...
    openGraph: { title: post.title, description: post.excerpt, type: 'article', url: `${site.url}/blog/${post.slug}` }
  };
}
...
export default async function Post({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const post = blogPosts.find((item) => item.slug === slug);
  if (!post) notFound();
  const citations = post.rich?.sources.map((source) => source.url) ?? sourceNotes.map((source) => source.url);
  const schema = [
    { '@context': 'https://schema.org', '@type': 'BlogPosting', '@id': `${site.url}/blog/${post.slug}#article`, headline: post.title, description: post.excerpt, url: `${site.url}/blog/${post.slug}`, datePublished: post.rich?.published, dateModified: post.rich?.updated, publisher: { '@id': `${site.url}/#organization` }, mainEntityOfPage: `${site.url}/blog/${post.slug}`, citation: citations, hasPart: post.sections.map((section, index) => ({ '@type': 'WebPageElement', position: index + 1, name: section.heading })) },
  ...
  return <><Header/><main className="section"><JsonLd data={schema}/><article className={`container article ${post.rich ? 'rich-article' : ''}`}><p className="eyebrow">{site.brand} guide</p><h1>{post.title}</h1><p className="lead">{post.excerpt}</p><div className="takeaway"><b>Key takeaway:</b> {post.takeaway}</div>{post.rich ? <RichPost post={post as BlogPost & { rich: NonNullable<BlogPost['rich']> }} /> : <><ArticleSections post={post} start={0} end={post.sections.length} /><section className="card"><h2>Questions people ask</h2>{post.faqs.map((faq) => <div key={faq.q} className="faq"><h3>{faq.q}</h3><p>{faq.a}</p></div>)}</section><section className="card"><h2>Reference notes</h2><p>These links are a starting point for general context. They are not custom legal, tax, hiring, or cybersecurity advice.</p><ul className="list">{sourceNotes.map((source) => <li key={source.url}><a href={source.url}>{source.name}</a>: {source.note}</li>)}</ul></section></>}</article>{post.rich ? null : <CTA/>}</main><Footer/></>;
}
