...
export default async function Post({ params }: { params: Promise<{ slug: string }> }) {
 ...
 return <><Header/><main className="section"><JsonLd data={schema}/><article className="container article"><p className="eyebrow">{site.brand} guide</p><h1>{post.title}</h1><p className="lead">{post.excerpt}</p><div className="takeaway"><b>Key takeaway:</b> {post.takeaway}</div>{post.sections.map((section)=><section className="card article-block" key={section.heading}><h2>{section.heading}</h2><p>{section.body}</p>{section.bullets ? <ul className="list">{section.bullets.map((b)=><li key={b}>{b}</li>)}</ul> : null}</section>)}<section className="card"><h2>Questions people ask</h2>{post.faqs.map((f)=><div key={f.q} className="faq"><h3>{f.q}</h3><p>{f.a}</p></div>)}</section><section className="card"><h2>Reference notes</h2><p>These links are a starting point for general context. They are not custom legal, tax, hiring, or cybersecurity advice.</p><ul className="list">{sourceNotes.map((s)=><li key={s.url}><a href={s.url}>{s.name}</a>: {s.note}</li>)}</ul></section></article><CTA/></main><Footer/></>;
}
